...
- 设置页已展示图片缓存、歌词缓存、模板缓存三类体积，并支持汇总后一键清理
- 图片缓存按 Coil 磁盘缓存目录统计；歌词缓存按 `lyrics_cache` 有效记录文本体积统计；模板缓存按内存模板内容估算
- 清理完成后立即刷新设置页数值，避免“清了但 UI 还傻站着不动”
...
- 本地歌单详情页提供“编辑 / 取消 / 完成”操作，非本地歌单不展示编辑入口
- 编辑态下支持长按拖拽排序与单曲移除，修改先保存在内存态，点击“完成”后批量提交到 Room
- 提交失败时保留编辑态并展示页内错误提示，避免直接把列表干成空白报错页
...
- 下载默认沿用播放器当前选中的音质，默认值保持为 `128k`
- `LOCAL` 平台和已落地到本地文件的歌曲不再进入下载链路
- 前端暂未拿到独立的“可下载 / 不可下载”字段时，以播放地址解析结果作为下载准入条件
- 下载解析失败时统一按“可能受音源或版权限制，当前不支持下载”处理，避免误导成客户端故障
