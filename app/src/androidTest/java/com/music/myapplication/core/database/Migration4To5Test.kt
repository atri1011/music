...
class Migration4To5Test {
    ...
    @Test
    fun migrate4To5AddsPlaylistRemoteSignaturesWithDefaultValue() {
        helper.createDatabase(TEST_DB, 4).apply {
            ...
            execSQL(
                """
                INSERT INTO `playlist_remote_map` (`playlist_id`, `source_platform`, `source_playlist_id`, `owner_uid`, `last_synced_at`)
                VALUES ('playlist-local-1', 'netease', '10001', '9527', 1700000001000)
                """.trimIndent()
            )
            close()
        }
        ...
        helper.runMigrationsAndValidate(TEST_DB, 5, true, MIGRATION_4_5).apply {
            query(
                """
                SELECT `source_playlist_id`, `remote_signature`, `last_synced_song_signature`
