...
- **必须模块化。** 页面、状态、业务编排、列表项、弹窗、底部栏等职责要拆开，避免一个 Screen 文件同时承载过多 UI 和逻辑。
- **一个组件一个文件。** 可复用的 Compose 组件默认独立成文件；较大的页面内部子区域也应尽早抽离，不要长期以内联大段 Composable 的形式堆在同一个文件里。
- **控制文件体量。** 手写源码建议优先控制在 **300 行以内**；超过后应先评估拆分；接近或超过 **500 行** 时原则上必须重构拆分，除非有明确理由。
- **Screen 负责编排，组件负责展示。** `Screen` 文件尽量只保留路由接线、状态收集、事件分发和布局骨架；复杂 UI 区块下沉到独立组件文件。
- **ViewModel 只保留核心状态管理。** 复杂映射、表单状态、排序策略、播放器附属逻辑等应提取到 reducer、mapper、use-case、delegate 或独立 state 文件中。
...
| 2026-04-30 | 更新 | 补充 Feature 模块开发规范，强调模块化、一个组件一个文件、控制文件体量 |
