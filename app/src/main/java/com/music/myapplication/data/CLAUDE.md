...
- **必须模块化。** Repository 实现优先做编排，不要同时塞入请求构建、平台分支、解析、缓存、补全、兜底和映射的全部细节。
- **一个协作者一个文件。** delegate、resolver、parser、enricher、mapper、cache helper 等默认独立文件，避免把多平台逻辑长期堆在一个超长实现类里。
- **控制文件体量。** 手写源码建议优先控制在 **300 行以内**；超过后先评估拆分；接近或超过 **500 行** 时原则上必须重构，除非确有必要。
- **Repository 负责编排，不负责膨胀。** 当平台差异、补全流程、错误兜底、批处理规则增长时，应继续下沉到专用 collaborator，而不是继续扩张主仓库类。
- **按平台与流程拆分。** 涉及网易云、QQ、酷我差异时，优先拆分为平台专属解析/补全/解析播放实现，避免出现难维护的大型 `when(platform)` 文件。
...
| 2026-04-30 | 更新 | 补充 Data 模块开发规范，强调仓库编排层职责与协作者拆分 |
